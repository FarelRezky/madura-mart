...
@section('content')
    ...
    <div class="container-fluid py-4">
                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-7 mb-3 mb-lg-0">
                                <div class="d-flex align-items-center mb-2">
                                    ...
                                    <h6 class="text-white opacity-8 mb-0 font-weight-normal text-uppercase tracking-widest" style="font-size: 0.75rem;">Edit Sale</h6>
                                </div>
                                <h1 class="text-white font-weight-bolder mb-2" id="detail_product_name" style="font-size: 2.5rem;">Editing...</h1>
                                <div class="d-flex align-items-center mt-2">
                                    <div id="stock_indicator" class="me-3 d-none d-flex align-items-center">
                                        <span class="stock-good"></span>
                                        <span class="text-white text-sm" id="stock_label">Ready to Process</span>
                                    </div>
                                    <span class="text-white opacity-8 text-sm mb-0" id="detail_calculation">Recalculating</span>
                                </div>
                            </div>
                            <div class="col-lg-5 text-lg-end">
                                <p class="text-white opacity-8 mb-1 text-xs text-uppercase font-weight-bold">Total Payment</p>
        ...
        <form action="{{ route('sales.update', $sale->id) }}" method="POST" id="form-sale">
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <div class="card shadow-sm border-0 mb-4" style="border-radius: 15px;">
                        <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
                            <div class="d-flex align-items-center mb-4">
                                ...
                                <h6 class="font-weight-bolder mb-0 text-dark">Edit Sale Detail</h6>
                            </div>
                            
                            <div class="row mb-3">
                                <div class="col-md-6 mb-3 mb-md-0">
                                    <label class="form-control-label text-uppercase text-xs font-weight-bolder text-secondary">Receipt Number</label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="fas fa-barcode"></i></span>
                                        <input type="text" class="form-control" id="sale_number" name="sale_number" value="{{ $sale->sale_number }}" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-control-label text-uppercase text-xs font-weight-bolder text-secondary">Sale Date</label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="fas fa-calendar-alt"></i></span>
                                        <input type="date" class="form-control" id="sale_date" name="sale_date" value="{{ \Carbon\Carbon::parse($sale->sale_date)->format('Y-m-d') }}" required>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body px-0 pt-0 pb-2">
                            <div class="table-responsive p-0 px-4">
                                <table class="table align-items-center mb-0" id="sale-table">
                                    <thead>
                                        <tr>
                                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 px-2" width="35%">Product</th>
                                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 px-2" width="20%">Unit Price</th>
                                            ...
                                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 px-2" width="10%">Delete</th>
                                        </tr>
                                    </thead>
                                    <tbody id="items-container">
                                        ...
                                        <tr class="item-row">
                                            <td class="p-2 border-0">
                                                <select name="products[{{ $index }}][serial]" class="form-select product-select fw-bold text-dark" required onchange="updatePrice(this)">
                                                    <option value="" disabled>Select Product...</option>
                                                    @foreach($products as $p)
                                                        @php
                                                            $adjustedStock = ($p->stock ?? $p->stok) + ($p->serial_number === $detail->product_serial ? $detail->qty : 0);
                                                        @endphp
                                                        <option value="{{ $p->serial_number }}" 
                                                                data-price="{{ $p->selling_price ?? $p->harga_jual }}" 
                                                                data-name="{{ $p->name ?? $p->nama_barang }}"
                                                                {{ $p->serial_number === $detail->product_serial ? 'selected' : '' }}>
                                                            {{ $p->name ?? $p->nama_barang }} (Stok: {{ $adjustedStock }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td class="p-2 border-0">
                                                <div class="input-group">
                                                    ...
                                                    <input type="number" name="products[{{ $index }}][price]" class="form-control price-input border-start-0 ps-0 text-dark" placeholder="0" value="{{ $detail->selling_price }}" required oninput="calcGrandTotal()">
                                                </div>
                                            </td>
                                            <td class="p-2 border-0">
                                                <input type="number" name="products[{{ $index }}][qty]" class="form-control qty-input text-center text-dark" value="{{ $detail->qty }}" min="1" required oninput="calcGrandTotal()">
                                            </td>
                                            <td class="p-2 border-0">
                                                <div class="input-group">
                                                    <span class="input-group-text border-end-0">Rp</span>
                            ...
                            <div class="px-4 py-3 mt-2">
                                ...
                                <input type="hidden" name="total_price" id="grand-total-input" value="{{ $sale->total_price }}">
                            </div>

                            <div class="row mt-3 px-4">
                                <div class="col-12 text-end pb-4 border-top pt-4">
                                    <a href="{{ route('sales.index')}}" class="btn btn-light mb-0 me-2 shadow-sm text-dark">Cancel</a>
    ...
    <script>
        let itemIndex = {{ count($sale->details) }};
        ...
        function calcGrandTotal() {
            ...
            if (itemCount > 0) {
                document.getElementById('detail_product_name').innerText = itemCount + " Products Selected";
                document.getElementById('detail_calculation').innerText = "Waiting for Payment...";
        ...
        function removeRow(btn) {
            if(document.querySelectorAll('.item-row').length > 1) {
                ...
                swal("Attention!", "There must be at least 1 product in the transaction!", "warning");
            }
        }
        ...
        document.getElementById('btn-simpan').addEventListener('click', function(e) {
            ...
            if (!firstSelect || total <= 0) {
                swal("Invalid", "Select a product and ensure the order quantity is correct!", "error");
                return;
            }

            swal({
                title: "Update Confirmation",
                text: "Total Billing: " + formatRupiah(total) + "\nSave changes transaction?",
                icon: "info",
                buttons: ["Cancel", "Save"],
                dangerMode: false,
            })
